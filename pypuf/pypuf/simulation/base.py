...
class LTFArray(Simulation):
    ...
    **Combiner Function.**
    ...
    @classmethod
    def normal_weights(cls, n: int, k: int, seed: int, mu: float = 0, sigma: float = 1,
                       *, T_factor: Optional[bool] = None, V_factor: Optional[bool] = None,
                       temperature: Optional[float] = None, vdd: Optional[float] = None) -> ndarray:
        """
    ...
    def ltf_eval(self, sub_challenges: ndarray) -> ndarray:
        ...
        # If no environmental variation requested, bypass scaling to guarantee deterministic results
        if not getattr(self, 'T_factor', False) and not getattr(self, 'V_factor', False):
            weights = self.weight_array[:, :-1]
            unbiased_sum = einsum('ji,...ji->...j', weights, sub_challenges, optimize=True)
            bias = self.weight_array[:, -1]
            # Do not scale bias when no physical factors are active
            return unbiased_sum + bias
...
class NoisyLTFArray(LTFArray):
    ...
    def __init__(self, weight_array, bias=None, transform=None, combiner='xor',
                 ...
                 temperature: Optional[float] = None, vdd: Optional[float] = None, m: float = 1.5, alpha: float = 1.2, T_factor: bool = None, V_factor: bool = None,

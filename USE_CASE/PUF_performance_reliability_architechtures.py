...
if __name__ == "__main__":
    # Reliability for different n at fixed temp (25°C) with voltage sweep (0.05V increments)
    sweep_ns = [32, 64, 128, 256]
    vdd_fine = np.round(np.arange(0.5, 3.001, 0.01), 3)  # 0.001V increments (ultra-fine)
    temp_fixed = 25
    num_reliability_crps = 10000
    print("\nArbiterPUF Reliability vs Vdd at 25°C:")
    plt.figure(figsize=(16,10))
    for n_bits in sweep_ns:
        print(f"  Challenge bits: {n_bits} (Vdd sweep)")
        reliability_challenges = np.random.choice([-1, 1], size=(num_reliability_crps, n_bits))
        reliabilities = []
        # Reference responses at default temp and vdd
        ref_puf = ArbiterPUF(n=n_bits, temperature=temp_fixed, vdd=1.35)
        ref_responses = ref_puf.eval(reliability_challenges)
        for vdd in vdd_fine:
            puf = ArbiterPUF(n=n_bits, temperature=temp_fixed, vdd=vdd)
        ...
        print(f"    Finished Vdd sweep for n={n_bits}")
        plt.plot(vdd_fine, reliabilities, label=f'n={n_bits}')
    plt.title('ArbiterPUF Reliability vs Vdd at 25°C', fontsize=20)
    ...
    # Reliability for different n at fixed voltage (1.3V) with temperature sweep
    temp_sweep = np.round(np.arange(0, 85.001, 0.1), 3)  # 0.01°C increments (ultra-fine)
    vdd_fixed = 1.3
    print("\nArbiterPUF Reliability vs Temperature at Vdd=1.3V:")
    plt.figure(figsize=(16,10))
    for n_bits in sweep_ns:
        print(f"  Challenge bits: {n_bits} (Temperature sweep)")
        reliability_challenges = np.random.choice([-1, 1], size=(num_reliability_crps, n_bits))
        reliabilities = []
        # Reference responses at default temp and vdd
        ref_puf = ArbiterPUF(n=n_bits, temperature=25, vdd=vdd_fixed)
        ref_responses = ref_puf.eval(reliability_challenges)
        for temp in temp_sweep:
            puf = ArbiterPUF(n=n_bits, temperature=temp, vdd=vdd_fixed)
            responses = puf.eval(reliability_challenges)
            reliability_val = np.mean(responses == ref_responses)
            reliabilities.append(reliability_val)
        print(f"    Finished temperature sweep for n={n_bits}")
        plt.plot(temp_sweep, reliabilities, label=f'n={n_bits}')
    plt.title('ArbiterPUF Reliability vs Temperature at Vdd=1.3V', fontsize=20)
